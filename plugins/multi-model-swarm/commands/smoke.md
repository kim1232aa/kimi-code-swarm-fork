...
After approval, call `AgentSwarm` once, as the only tool call in that response, with a non-empty `description`, `prompt_template: "{{item}}"`, and exactly two object items. Bind the first item to the first alias and the second item to the second alias. Each task should return a short route marker and no tool calls. Report tool success or the exact routing error; never claim success from the requested alias alone.
