...
// Hook payloads are external input. These checks are deliberately defensive;
// the AgentSwarm schema remains the source of truth for full validation.
const toolInput = payload?.tool_input ?? payload?.toolInput;
if (!isRecord(toolInput)) process.exit(0);
...
if (itemEntries.length > 0) {
  if (typeof promptTemplate !== 'string' || promptTemplate.trim().length === 0) {
    deny(
      'AgentSwarm items require prompt_template: "{{item}}". Example: { "prompt_template": "{{item}}", "items": ["task A", "task B"] }.',
    );
  }
...
for (const [index, value] of itemEntries.entries()) {
  ...
  if (isRecord(value)) hasObjectItems = true;
