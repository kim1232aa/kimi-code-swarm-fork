...
type PayloadStyle = 'snake_case' | 'camelCase';

function runHook(
  toolInput: unknown,
  capabilities?: string,
  payloadStyle: PayloadStyle = 'snake_case',
): { readonly allowed: boolean; readonly reason?: string } {
  const payload =
    payloadStyle === 'snake_case'
      ? { tool_name: 'AgentSwarm', tool_input: toolInput }
      : { toolName: 'AgentSwarm', toolInput };
...
describe('multi-model AgentSwarm PreToolUse guard', () => {
  it.each([
    ...
    },
    {
      ...
      input: { prompt_template: '{{item}}', items: ['Model A（provider/model-a）', 'task B'] },
      reason: 'looks like a model label',
    },
    {
      name: 'a malformed items value',
      input: { prompt_template: '{{item}}', items: 'task A' },
      reason: 'items must be an array',
    },
  ])('defensively denies $name for a snake_case hook payload', ({ input, reason }) => {
    const result = runHook(input, CAPABILITY);

    expect(result).toEqual({ allowed: false, reason: expect.stringContaining(reason) });
  });

  it('allows a snake_case resume-only payload without a prompt template', () => {
    expect(
      runHook({
  ...
  it('allows snake_case object items when the fork capability is available', () => {
    expect(
      runHook(
        {
  ...
  it('denies snake_case object items when the fork capability is unavailable', () => {
  ...
  it('keeps camelCase payload compatibility', () => {
    expect(
      runHook(
        {
          prompt_template: '{{item}}',
          items: ['Review the API', 'Review the tests'],
        },
        CAPABILITY,
        'camelCase',
      ),
